...
- [前后端通信](./communication.md) — 本地 IPC 与外部 HTTP 分层、Command 契约、错误与事件
...
- 前端外部 HTTP 请求统一走 `src/api/http/request.ts` 的 `request<T>()`，不要直接 `fetch`。
- 前端 Tauri IPC 调用统一走 `src/api/ipc/client.ts` 的 `ipcInvoke<T>()` 和 `src/api/ipc/modules/`，不要在组件或 store 中直接 `invoke`。
