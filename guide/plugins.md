...
tauri-zero 已集成常用 Tauri 官方插件。IPC 相关能力封装在 `src/api/modules/`，插件工具函数保留在 `src/utils/`。

## 文件系统

文件读写通过 Rust 命令实现（`src-tauri/src/platform/fs.rs`），前端封装在 `src/api/modules/fs.ts`：

```ts
import { readTextFile, writeTextFile, fileExists } from "../api/modules/fs";
